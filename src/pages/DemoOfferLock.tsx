...
export default function DemoOfferLock() {
  const [form, setForm] = useState({ name:'', level:'College', schoolOrPlatform:'', followers:'', brand:'', offerValue:'' });
  const [locked, setLocked] = useState(null);
  const onChange = (e:any)=>setForm((f:any)=>({ ...f, [e.target.name]: e.target.value }));
  const lockOffer = (e:any)=>{ e.preventDefault(); setLocked({ id:'OL-'+Math.random().toString(36).slice(2,8).toUpperCase() }); };
  ...
  if (locked) return (
    <div style={{maxWidth:720,margin:'2rem auto',padding:16}}>
      ...
      <div style={{display:'flex',gap:12,marginTop:16}}>
        <Link to="/demo/contract" style={{padding:'8px 12px',border:'1px solid #333',borderRadius:8}}>Continue → Contract</Link>
  ...
  return (
    <div style={{maxWidth:720,margin:'2rem auto',padding:16}}>
      ...
      <p style={{color:'#555',marginBottom:16}}>Quick, client-only mock to capture an NIL offer.</p>
      <form onSubmit={lockOffer} style={{display:'grid',gap:12}}>
        ...
        <label>Level
          <select name="level" value={form.level} onChange={onChange} style={input}>
            <option>HS</option><option>College</option><option>Pro</option><option>Creator</option>
          </select>
        </label>
        ...
        <button type="submit" style={btn}>Lock Offer</button>
      </form>
      <div style={{marginTop:16}}><Link to="/" style={{padding:'8px 12px'}}>Back Home</Link></div>
    </div>
  );
}
const input:any = {display:'block',width:'100%',marginTop:6,padding:'10px 12px',border:'1px solid #ccc',borderRadius:8};
const btn:any   = {padding:'10px 14px',border:'1px solid #111',background:'#111',color:'#fff',borderRadius:10,cursor:'pointer'};
